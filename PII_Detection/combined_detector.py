...
class CombinedPIIDetector:
    ...
    def detect_table_batch(
        self,
        table_schemas: Any,
        sample_data: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
        """
        Batch detect PII across table columns.
        """
        results = {}
        if isinstance(table_schemas, dict):
            tables = table_schemas
        elif isinstance(table_schemas, list):
            tables = {item.get("table_name", f"table_{i}"): item for i, item in enumerate(table_schemas) if isinstance(item, dict)}
        else:
            tables = {}

        for tbl_name, tbl_info in tables.items():
            cols = tbl_info.get("columns", []) if isinstance(tbl_info, dict) else (tbl_info if isinstance(tbl_info, list) else [])
            tbl_results = []
            samples = (sample_data or {}).get(tbl_name, {})
            
            for col in cols:
                col_name = col.get("column_name") if isinstance(col, dict) else str(col)
                dtype = col.get("data_type", "VARCHAR") if isinstance(col, dict) else "VARCHAR"
                sample_vals = samples.get(col_name, []) if isinstance(samples, dict) else []
                
                res = self.detect_column(
