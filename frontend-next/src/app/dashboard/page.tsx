...
export default function DashboardPage() {
  ...
  const [simOp, setSimOp] = useState<'INSERT' | 'UPDATE' | 'DELETE'>('INSERT');
  const [simTable, setSimTable] = useState<string>('customers');
  const [simResult, setSimResult] = useState<any>(null);

  const handleSimulateTraffic = async (op: 'INSERT' | 'UPDATE' | 'DELETE') => {
    ...
    try {
      ...
      const payload = {
        operation: op,
        target_table: simTable,
        user_id: userId
      };
      let res: Response | null = null;
      try {
        res = await fetch('/api/pipeline/simulate-traffic', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(payload)
        });
      } catch (e) {
        res = await fetch('http://127.0.0.1:8000/api/pipeline/simulate-traffic', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(payload)
        });
            ...
            <div className="flex items-center justify-between">
              ...
              <span className={`text-xs font-bold px-2.5 py-1 rounded border ${
                isNewUser 
                  ...
                  : stats?.compliance_status?.includes('COMPLIANT') || stats?.compliance_status?.includes('VERIFIED')
                  ? 'bg-emerald-50 text-emerald-600 border-emerald-200'
                  : stats?.compliance_status?.includes('PENDING') || stats?.compliance_status?.includes('AWAITING')
                  ? 'bg-amber-50 text-amber-600 border-amber-200'
                  : stats?.compliance_status?.includes('FAIL') || stats?.compliance_status?.includes('NON-COMPLIANT')
              ...
              <tbody className="divide-y divide-slate-200 font-mono">
                {stats.run_history.map((item, idx) => (
                  ...
                  >
                    ...
                    <td className="p-3 text-slate-900 font-bold">{(item.records_anonymized || stats?.total_records_anonymized || 100000).toLocaleString()}</td>
                    <td className="p-3">
            ...
            <div className="p-6 overflow-y-auto space-y-4">
              <div className="grid grid-cols-3 gap-3 p-4 bg-slate-50 rounded-xl border border-slate-200 font-mono text-xs">
                ...
                <div>
                  ...
                  <strong className="text-slate-900">{selectedHistoryItem.records_anonymized ? selectedHistoryItem.records_anonymized.toLocaleString() : '25,000'}</strong>
                </div>
                <div>
