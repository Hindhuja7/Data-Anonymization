    ...
    <div className="p-6 max-w-6xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-white">Enterprise Domain Classification</h1>
            <p className="text-xs text-slate-400">Authoritative Step-3 AI Domain Detection</p>
          </div>
        </div>
      </div>
      ...
      {!enterpriseType ? (
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-12 text-center space-y-4">
          ...
          <h3 className="text-sm font-bold text-slate-300">No Enterprise Analysis Available</h3>
          <p className="text-xs text-slate-500 max-w-md mx-auto">
            No enterprise detection analysis has been performed for the current run <span className="font-mono text-slate-300">({runId})</span> yet. Start the pipeline to execute Step 3.
          </p>
        </div>
      ) : (
        <>
          ...
          <div className="bg-slate-900 border border-slate-800 p-4 rounded-xl flex items-center justify-between font-mono text-xs">
            ...
            <div>
              ...
              <span className="text-blue-400 font-bold">{targetTable}</span>
            </div>
            <div>
              <span className="text-slate-500 block text-[10px] uppercase">Detection Status</span>
              <span className="text-emerald-400 font-bold">COMPLETED (STEP 03)</span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 space-y-2">
              ...
              <p className="text-xl font-extrabold text-blue-400 font-mono tracking-wide uppercase">{enterpriseType}</p>
              <p className="text-[11px] text-slate-400">Target Table Domain Classification ({sourceLabel})</p>
            </div>
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 space-y-2">
              ...
              <p className="text-xl font-extrabold text-emerald-400 font-mono tracking-wide">{confidence ? `${confidence}%` : '—'}</p>
              <p className="text-[11px] text-slate-400">{confidenceSubtitle}</p>
            </div>
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 space-y-2">
              ...
              <p className="text-sm font-bold text-purple-300 font-mono leading-tight">{complianceLaw}</p>
              <p className="text-[11px] text-slate-400">Mandatory Statutory Regulation</p>
            </div>
          </div>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 space-y-4">
            <div className="flex items-center gap-2 pb-3 border-b border-slate-800">
              ...
              <h2 className="text-sm font-bold text-white uppercase tracking-wider">Step-3 Enterprise Analysis Rationale</h2>
            </div>

            <div className="bg-slate-950 p-4 rounded-xl border border-slate-800/80 leading-relaxed text-xs font-mono text-slate-300 space-y-2">
              <p className="text-blue-400">// Authoritative Step 3 Output Log for table '{targetTable}':</p>
              <p>{reasoning}</p>
            </div>
          </div>
        </>
