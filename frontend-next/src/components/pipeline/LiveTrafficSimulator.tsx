...
export const LiveTrafficSimulator: React.FC<LiveTrafficSimulatorProps> = ({
  ...
  useEffect(() => {
    if (activeTable) {
      const fetchSchema = async () => {
        ...
        try {
          const res = await apiFetch(`/api/pipeline/table-schema?table=${activeTable}`);
          if (res && res.ok) {
            const data = await res.json();
  ...
  const fetchRecords = async () => {
    ...
    try {
      const res = await apiFetch(`/api/pipeline/table-records?table=${activeTable}&limit=20`);
      if (res && res.ok) {
        const data = await res.json();
  ...
  const handleSimulate = async (operation: 'INSERT' | 'UPDATE' | 'DELETE') => {
    ...
    try {
      let activeUserId = 'b@gmail.com';
      if (typeof window !== 'undefined') {
        try {
          const stored = localStorage.getItem('datavault_user');
          if (stored) {
            const parsed = JSON.parse(stored);
            if (parsed.email || parsed.id) activeUserId = parsed.email || parsed.id;
          }
        } catch (e) {}
        if (activeUserId === 'b@gmail.com') {
          activeUserId = localStorage.getItem('datavault_user_email') || localStorage.getItem('datavault_user_id') || localStorage.getItem('datavault_active_user') || 'b@gmail.com';
        }
      }
      ...
      const response = await apiFetch('/api/pipeline/simulate-traffic', {
        method: 'POST',
