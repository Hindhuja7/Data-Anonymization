...
class WebSocketService:
    ...
    async def broadcast_log(self, log: Any, level: str = "info"):
        ...
        if isinstance(log, dict):
            log_data = {
                ...
                "message": log.get("details") or log.get("action") or str(log),
                "level": log.get("level") or level
            }
        else:
            log_data = {
                "type": "log",
                "data": {"id": f"log_{int(datetime.now().timestamp()*1000)}", "action": str(log), "details": str(log), "level": level, "timestamp": datetime.now().isoformat()},
