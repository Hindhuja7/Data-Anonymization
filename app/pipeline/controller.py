...
class PipelineController:
    ...
    async def start_pipeline(self, user_id: Optional[str] = None, database_config: Optional[dict] = None, target_table: Optional[str] = None) -> dict:
        ...
        try:
            ...
            db_type = str(db_conf.get("database_type") or db_conf.get("type") or "mysql").lower()
            default_db_name = "defaultdb" if db_type == "mysql" else "neondb"
            database_name = db_conf.get("database_name") or db_conf.get("database") or db_conf.get("dbname") or default_db_name
            dest_database_name = db_conf.get("dest_database_name") or (f"{database_name}_anonymized" if not str(database_name).endswith("_anonymized") else database_name)
            ...
            pipeline_state.set("dest_database_name", dest_database_name)
            if user_id:
            ...
            def run_pipeline():
                try:
                    ...
                    if self.cancel_event and self.cancel_event.is_set():
                        ...
                        # Automatically record run in user's run_history log with exact live table record count
                        try:
                            from app.services.audit_service import audit_service
                            act_uid = pipeline_state.get("user_id")
                            r_id = pipeline_state.get("run_id")
                            tbl_n = pipeline_state.get("target_table") or target_tbl
                            pol_d = pipeline_state.get("approved_policy") or pipeline_state.get("generated_policy") or {}
                            if isinstance(pol_d, dict):
                                pol_d["total_records_anonymized"] = audit_service._get_table_record_count(tbl_n)
                            ...
                            logger.warning(f"Error recording completed run history: {rh_err}")
