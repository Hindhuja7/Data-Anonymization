...
A privacy-preserving PII detection pipeline for databases with enterprise-aware context and sequential model fallback. The system:
1. Connects to databases (PostgreSQL, MySQL, SQL Server)
2. Extracts table schemas and sample data
3. Detects enterprise type (BANKING, HR, etc.) for context-aware detection
4. Detects PII using LLM-based and regex-based approaches
5. Generates synthetic Indian enterprise test data
6. Loads data into Neon PostgreSQL for testing
7. Applies anonymization techniques to PII data
...
| Test Data Generator | `generate_test_data.py` | Generates synthetic Indian enterprise test data |
| Neon Loader | `load_to_neon.py` | Loads CSV data into Neon PostgreSQL |
...
**Option A: Generate test data and run detection**
```bash
...
**Option B: Run detection on existing database**
```python
...
>>>>>>> 38bb0155ab881ac220d79833bccf2f93ce4cf3da
