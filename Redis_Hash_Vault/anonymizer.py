...
class Anonymizer:
    ...
    def _mask(
        ...
        for value in values:
            ...
            col_low = (column_name or "").lower()

            if "date" in col_low or "time" in col_low or re.match(r'^\d{4}-\d{2}-\d{2}', value_str):
                # Type-aware SQL DATE masking: 2023-11-10 -> 2023-01-01
                if len(value_str) >= 10 and re.match(r'^\d{4}-\d{2}-\d{2}', value_str):
                    masked_value = value_str[:4] + "-01-01" + value_str[10:]
                else:
                    masked_value = "2024-01-01"
            elif pii_type == "AADHAAR" or "aadhaar" in col_low:
            ...
            elif pii_type == "PAN" or "pan" in col_low:
                if len(clean_value) >= 4:
                    masked_value = clean_value[:-4] + "XXXX"
                else:
                    masked_value = "X" * len(clean_value)
            
            elif pii_type == "GSTIN" or "gstin" in col_low:
                if len(clean_value) >= 5:
                    masked_value = clean_value[:-5] + "XXXXX"
                else:
                    masked_value = "X" * len(clean_value)
            
            elif pii_type == "CREDIT_CARD" or "credit" in col_low:
                if len(clean_value) >= 4:
                    masked_value = "X" * (len(clean_value) - 4) + clean_value[-4:]
                else:
                    masked_value = "X" * len(clean_value)
            
            elif pii_type == "BANK_ACCOUNT" or "account" in col_low:
                if len(clean_value) >= 4:
                    masked_value = "X" * (len(clean_value) - 4) + clean_value[-4:]
                else:
                    masked_value = "X" * len(clean_value)
            
            elif pii_type == "DRIVING_LICENSE" or "license" in col_low:
                if len(clean_value) >= 4:
                    masked_value = clean_value[:-4] + "XXXX"
                else:
                    masked_value = "X" * len(clean_value)
            
            elif pii_type == "UAN" or "uan" in col_low:
                if len(clean_value) >= 4:
                    masked_value = clean_value[:-4] + "XXXX"
                else:
                    masked_value = "X" * len(clean_value)
                ...
                if isinstance(value, (int, float)) or (isinstance(value_str, str) and value_str.isdigit()):
                    masked_value = int(value_str[0] + "0" * (len(value_str) - 1)) if value_str.isdigit() else 0
